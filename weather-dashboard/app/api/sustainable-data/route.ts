...
export async function GET() {
  try {
    ...
    for (const file of files) {
      ...
      if (periodMatch) {
        datePrefix = periodMatch[1].trim(); // e.g. "26/2"
      }
      ...
      for (let i = 3; i < lines.length; i++) {
        ...
        const energy = parseFloat(cols[1]);
        const current = parseFloat(cols[2]);
        ...
        const timeString = cols[0]; // e.g "0:05"

        const entry = {
          time: `${datePrefix} ${timeString}`,
          timeOnly: timeString, // useful for comparison chart X axis
          energy,
          current,
        ...
        maxPower = Math.max(maxPower, current * voltage / 1000);
        count++;
        ...
        groupedStats[datePrefix].maxPower = Math.max(groupedStats[datePrefix].maxPower, current * voltage / 1000);
        groupedStats[datePrefix].count++;
      }
    }

    // Format grouped stats properly
